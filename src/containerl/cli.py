...
def test_connection(server_address: str, num_steps: int = 5, agent_mode: bool = False) -> None:
    ...
    try:
        if agent_mode:
            ...
            from containerl.interface.agent.client import main as client_main
            print(f"Testing agent connection to {server_address}...")
        else:
            # Import the environment client module's main function
            from containerl.interface.environment.client import main as client_main
...
def remove_containerl_images() -> None:
    """
    Remove all Docker images that start with 'containerl-'.
    """
    try:
        ...
        if not images or (len(images) == 1 and not images[0]):
            print("No containerl images found.")
            return

        # Remove each image
        print(f"Found {len(images)} containerl images. Removing...")
        for image in images:
            try:
        ...
        print("Finished removing containerl images.")
    except Exception as e:
        print(f"Error removing containerl images: {str(e)}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    # Test command
    test_parser = subparsers.add_parser("test", help="Test connection to a ContaineRL environment or agent server")
    ...
    subparsers.add_parser("remove-images",
                         help="Remove all Docker images that start with 'containerl-'")
