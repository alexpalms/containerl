...
class Environment(CRLVecGymEnvironment):
    """A simple discrete action vectorized environment with dictionary observations."""
    ...
    def step(
        ...
        # Expect action shape to be (num_envs,)
        if action.shape != (self.num_envs,):
            raise Exception(
                f"Action shape must be ({self.num_envs}, ), got {action.shape}"
            )
