"""A vectorized environment with continuous action space and dictionary observations."""
...
class Environment(CRLVecEnvironment[NDArray[np.floating]]):
    """A simple continuous action vectorized environment with dictionary observations."""
    ...
    def step(
        ...
        # Expect action shape to be (num_envs, 10)
        if action.shape != (self.num_envs, 10):
            raise Exception(
                f"Action shape must be ({self.num_envs}, 10), got {action.shape}"
            )
